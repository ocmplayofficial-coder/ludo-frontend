  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const extractedUpiId = pmData.upiId || pmData.paymentMethod?.upiId || '';
        const extractedId = pmData._id || pmData.paymentMethod?._id || '';

        setUpiId(extractedUpiId);
        setPaymentMethodId(extractedId);

        // Create order for the specific amount
        const orderRes = await fetch(`${API_URL}/api/payment/create-order`, {
          ...
          body: JSON.stringify({ amount: Number(amount), paymentMethodId: extractedId })
        });
        ...
        // Extract QR Code (supports order response or payment method response variations)
        const qr = orderData.order?.qrCode || pmData.qrCode || pmData.qrImageUrl || pmData.paymentMethod?.qrImageUrl || pmData.paymentMethod?.qrCode || '';
        if (qr) {
          // encodeURI ensures spaces in filenames (like "Visual QR.jpeg") are correctly escaped for src=""
          setQrCodeUrl(qr.startsWith('http') ? encodeURI(qr) : `${API_URL}${encodeURI(qr)}`);
        }
      } catch (e: any) {
        ...
        <div className="bg-neutral-900/40 border border-rose-955/15 p-5 rounded-2xl flex flex-col items-center space-y-5">
          ...
          {loading ? (
            ...
            <div className="w-48 h-48 rounded-xl bg-neutral-950/80 border border-amber-500/10 flex flex-col items-center justify-center text-center p-4 space-y-2">
              <QrCode className="w-12 h-12 text-amber-500/40" />
              <span className="text-[11px] text-zinc-400 font-medium">Scan code or use UPI ID below to pay</span>
            </div>
          )}
        </div>
